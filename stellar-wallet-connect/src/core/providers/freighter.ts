...
export class FreighterProvider extends BaseWalletProvider {
  readonly id = "freighter";
  readonly name = "Freighter";
  readonly type = "extension" as const;
  protected readonly kitWalletId = "freighter";

  async isAvailable(): Promise<boolean> {
    if (typeof window === "undefined") return false;
    try {
      const wallets = await kit.getSupportedWallets();
      const wallet = wallets.find((w) => w.id === this.kitWalletId);
  ...
  protected async doConnect(): Promise<ProviderConnectionResult> {
    ...
    try {
      const { address } = await kit.getAddress({ skipRequestAccess: false });
