...
export class LedgerService {
  ...
  async getPortfolioSummary(walletAddress: string) {
    ...
    for (const action of confirmedActions) {
      ...
      const verified = action.verifiedPayload as Record<string, any> | null;
      const amountSource = isPayoutAction && verified ? verified : payload;
      const amount = Number(amountSource.amount ?? payload.amount ?? 0);
      ...
      if (action.actionType === "deposit") {
        ...
        // A claim confirmed on-chain but with no verified payload on record
        // (shouldn't happen — both reconciliation paths always set it for a
        // confirmed row — but if data predates this column, or the event
        // decode failed silently upstream) is excluded from the total rather
        // than trusting an unverified actionPayload amount.
